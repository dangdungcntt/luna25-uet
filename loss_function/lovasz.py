...
class LovaszBCELoss(LovaszLoss):
    def __init__(self, per_image=True, ignore=None, weight=None, pos_weight=None):
        super().__init__(per_image, ignore)
        self.bce_loss = torch.nn.BCEWithLogitsLoss(weight=weight, pos_weight=pos_weight)

    def forward(self, logits, labels):
        lovasz = super().forward(logits, labels)
        bce = self.bce_loss(logits, labels.float())
        return (lovasz + bce) * 0.5
