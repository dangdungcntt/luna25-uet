...
class MalignancyProcessor:
    ...
    def __init__(self, mode="3D", model_root="/opt/app/resources/", 
                 ...
                 size_px=64, size_mm=50, suppress_logs=False, device="cpu"):
        ...
        # ]
        self.fold_paths = [os.path.join(self.model_root, self.model_name)]
        if self.mode == "2D":
    ...
    def _build_2d_model(self):
        self.base_model = ResNet18(weights=None).to(self.device).eval()

    def _build_3d_model(self):
        # only classification model, no segmentation
        self.base_cls = I3D_UNet(
            num_classes=1,
            input_channels=3,
            out_channels=1,
            pre_trained=True,
    ...
    def extract_patch(self, coord, output_shape, mode):
        patch, _ = dataloader.extract_patch(
            CTData=self.image,
            coord=coord,
    ...
    def _predict_fold(self, fold_path):
        ...
        tensor = torch.from_numpy(nodules)

        if self.mode == "3D":
            tensor = torch.nn.functional.interpolate(
                tensor, 
                size=(64, int(64 * 2), int(64 * 2))
            )
        else:
            tensor = torch.nn.functional.interpolate(
                tensor, 
                size=(int(64 * 2), int(64 * 2))
            )
        tensor = tensor.to(self.device)
        if self.mode == "2D":
            model = self.base_model
            # load checkpoint for classification
            ckpt = torch.load(os.path.join(fold_path, 'best_ckpt.pth'), map_location=self.device)['state_dict']
