...
class NoduleExtractor:
    ...
    def process_seriesuid(
        ...
        mask_path = str(self.mask_root / f"{seriesuid}.nii.gz")
            ...
            if Path(output_path).is_file():
                ...
                if Path(image_path).is_file():
                    ...
                    for i, coord in enumerate(coords):
                        ...
                        if image_patch.GetSize() == tuple(self.patch_size) and mask_patch.GetSize() == tuple(self.patch_size):
                            mask_patch, header = utils.itk_image_to_numpy_image(mask_patch)

                            annotation_id = pd.AnnotationID

                            dst_image_path = self.output_path / "image"
                            dst_metadata_path = self.output_path / "metadata"
                            dst_image_path.mkdir(exist_ok=True, parents=True)
                            dst_metadata_path.mkdir(exist_ok=True, parents=True)

                            np.save(dst_image_path / f"{annotation_id}.npy", mask_patch)
                            np.save(dst_metadata_path / f"{annotation_id}.npy", header)
                        else:
...
if __name__ == "__main__":
    ...
    extractor = NoduleExtractor(
        csv_path=Path(
            "data/luna25/LUNA25_Public_Training_Development_Data.csv"
        ),
        image_root=Path("data/luna25/luna25_images"),
        mask_root=Path("data/LUNA25-MedSAM2/instance-labels"),
        output_path=Path(
            "data/luna25/luna25_mask_blocks"
        ),
    )
