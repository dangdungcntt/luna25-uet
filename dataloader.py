...
from preprocessing.utils import rot_flip_xy_2d, rot_flip_3d, rot_flip_xy_3d, rot_flip_yz_3d
...
class CTCaseDataset(data.Dataset):
    ...
    def __init__(
        ...
        size_mm: int = 50,
        mode: str = "2D",
        flip_probs: list = [0, 0, 0],
        interpolate_scale: int = 1,
        ...
        if mode == "2D":
            self.flip_functions = [rot_flip_xy_2d]
        else:
            self.flip_functions = [rot_flip_3d, rot_flip_xy_3d, rot_flip_yz_3d]
        self.flip_probs = flip_probs[: len(self.flip_functions)]
        ...
        if self.use_monai_transforms:
            if self.mode == "2D":
                self.transforms = Compose([
                    RandGaussianNoised(prob=0.3, mean=0.0, std=0.1, keys=["image"]),
                    RandAdjustContrastd(prob=0.3, gamma=(0.8, 1.2), keys=["image"]),
                    RandGaussianSmoothd(prob=0.3, sigma_x=(0.5, 1.5), keys=["image"]),
                    RandScaleIntensityd(prob=0.3, factors=0.3, keys=["image"]),
                    RandShiftIntensityd(prob=0.3, offsets=0.1, keys=["image"]),
                    RandZoomd(prob=0.3, min_zoom=0.9, max_zoom=1.1, keys=["image", "mask"]),
                    RandFlipd(prob=0.5, spatial_axis=0, keys=["image", "mask"]),
                    RandFlipd(prob=0.5, spatial_axis=1, keys=["image", "mask"]),
                    ScaleIntensityRanged(a_min=-1000.0, a_max=600.0, b_min=0.0, b_max=1.0, keys=["image"]),
                ])
            else:  # 3D mode
                self.transforms = Compose([
                    RandGaussianNoised(prob=0.4, mean=0.0, std=0.15, keys=["image"]),
                    RandAdjustContrastd(prob=0.4, gamma=(0.7, 1.3), keys=["image"]),
                    RandGaussianSmoothd(prob=0.3, sigma_x=(0.5, 1.5), sigma_y=(0.5, 1.0), sigma_z=(0.5, 1.0), keys=["image"]),
                    RandScaleIntensityd(prob=0.3, factors=0.3, keys=["image"]),
                    RandShiftIntensityd(prob=0.3, offsets=0.1, keys=["image"]),
                    RandHistogramShiftd(prob=0.4, num_control_points=10, keys=["image"]),
                    RandBiasFieldd(prob=0.5, degree=2, keys=["image"]),
                    RandZoomd(prob=0.4, min_zoom=0.8, max_zoom=1.2, keys=["image", "mask"]),
                    RandFlipd(prob=0.5, spatial_axis=0, keys=["image", "mask"]),
                    RandFlipd(prob=0.5, spatial_axis=1, keys=["image", "mask"]),
                    RandFlipd(prob=0.5, spatial_axis=2, keys=["image", "mask"]),
                    ScaleIntensityRanged(a_min=-1000.0, a_max=600.0, b_min=0.0, b_max=1.0, keys=["image"]),
                ])
    ...
    def __getitem__(self, idx):
        ...
        mask_path = self.mask_dir / "image" / f"{annotation_id}.npy"
        ...
        voxel_spacing = [self.size_mm / self.size_px] * 3 if self.size_mm is not None else None
        patch, mask_patch = extract_patch(
            CTData=img,
            mask=mask,
        ...
        if self.use_monai_transforms:
            ...
            patch = transformed['image'].numpy()
            mask_patch = transformed['mask'].numpy()
        else:
        ...
        for prob, function in zip(self.flip_probs, self.flip_functions):
            if random.random() < prob:
                patch, mask_patch = function(patch, mask_patch)

        if self.interpolate_scale != 1:
            if self.mode == "3D":
                patch = torch.nn.functional.interpolate(
                    patch.unsqueeze(0), 
                    size=(64, int(64 * self.interpolate_scale), int(64 * self.interpolate_scale))
                ).squeeze(0)
            else:
                patch = torch.nn.functional.interpolate(
                    patch.unsqueeze(0), 
                    size=(int(64 * self.interpolate_scale), int(64 * self.interpolate_scale))
                ).squeeze(0)
...
def extract_patch(
    ...
    if mode == "2D":
        ...
        mask_patch = np.expand_dims(mask_patch, axis=0) if mask_patch is not None else None

        # patch = np.repeat(patch, 3, axis=0)
        # mask_patch = np.repeat(mask_patch, 1, axis=0)
