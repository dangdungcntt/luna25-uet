...
class I3D(torch.nn.Module):
    def __init__(
        ...
        self.classifier = torch.nn.Sequential(
            OrderedDict([
                ("pool", torch.nn.AdaptiveAvgPool3d((1, 1, 1))),
                ("dropout", torch.nn.Dropout(dropout_prob)),
                ("flatten", torch.nn.Flatten(1)),
                ("fc", torch.nn.Linear(1024, self.num_classes))
        ]))

    def forward(self, inp, extract_features = False):
        if self.input_channels == 3 and inp.shape[1] == 1:
            inp = inp.expand(-1, 3, -1, -1, -1)
        # Preprocessing
        out0 = self.conv3d_1a_7x7(inp)
        out = self.maxPool3d_2a_3x3(out0)
        out = self.conv3d_2b_1x1(out)
        
        out1 = self.conv3d_2c_3x3(out)
        out = self.maxPool3d_3a_3x3(out1)
        out = self.mixed_3b(out)
        
        out2 = self.mixed_3c(out)
        out = self.maxPool3d_4a_3x3(out2)
        out = self.mixed_4b(out)
        out = self.mixed_4c(out)
        out = self.mixed_4d(out)
        out = self.mixed_4e(out)
        
        out3 = self.mixed_4f(out)
        out = self.maxPool3d_5a_2x2(out3)
